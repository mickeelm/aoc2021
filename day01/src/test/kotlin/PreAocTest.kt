...
internal class PreAocTest {
    ...
    @Test
    internal fun `solutionPart1 should work with real input`() {
        assertEquals(3590905, solutionPart1(File("input.txt").readLines().map { it.toInt() }))
    }

    @Test
    internal fun `solutionPart2 should work with real input`() {
        assertEquals(1047, solutionPart2(File("input.txt").readLines().map { it.toInt() }))
    }
}